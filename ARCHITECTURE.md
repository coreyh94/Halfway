...
`WorkspaceSwitchCoordinator` serializes switch requests through a task tail without holding a synchronization lock across confirmation, terminal stop, persistence, presentation, or process launch. It resolves targets by stable ID into a separate prevalidated `WorkspaceCatalog`, asks the presentation adapter for ownership/partial-input confirmation, invokes exact coordinator bulk stop, and uses `OrderlyShutdownPersistence.FlushAsync` as a non-disposing barrier. Only after those operations succeed does it invalidate the old activation, replace the active presentation, mark the target recent, and start the target selections.

`WorkspaceActivationGeneration` supplies an explicit window activation token. Output, state presentation, accepted-input completion, resize, notification, completion-deadline, alert, and dispatcher work capture and revalidate that token. Durable final writes remain bound to their original session IDs, while stale presentation work cannot address a replacement workspace. `SessionRegistry.Clear` is used only after old ownership and persistence settle, so it contains exactly the active catalog across A -> B -> A. The completion scheduler and prepared alert presentation are replaced per activation; durable pending alert facts remain in SQLite for the exact workspace parent.

`SessionCoordinator` reconciles live ownership from the exact owned `ITerminalSession` exit event and completion task; it never scans arbitrary operating-system processes or adopts a PID. One coordinator ownership gate protects every session-map access, terminal generation check, readiness observation, lifecycle transition, write start, and post-write readiness reset. A zero exit remains Completed, a nonzero exit remains Failed, and an explicit stop or cancelled exit remains Disconnected. If completion ends without an exit fact, normal or cancelled completion conservatively becomes Disconnected while a faulted completion becomes Failed. Once ownership is released, output and writes carrying that generation cannot mutate registry state or target a replacement, repeated callbacks create no duplicate lifecycle event or eligible alert, and the same metadata may later start only through an explicit existing start action. Known closed-handle and closed-pipe cleanup races are contained only after the owned terminal is proven complete; other teardown failures are not blanket-suppressed. Reconciliation does not persist process identity, reattach, automatically restart, or manufacture terminal input.
...
User input queuing and automatic alert input are deliberately separate channels, but both acquire the same per-ownership asynchronous input gate before reaching one terminal. Alerts continue through `AlertInputCoordinator` and the durable `Pending`/`Reserved`/`Delivered` ledger, remain blocked by Planner partial input and readiness, and are not placed in `SubmittedInputQueue`. After durable reservation they revalidate the exact parent, session generation, terminal, readiness, and partial-input state immediately before the write. The user queue is bounded process memory only: prompts, partial input, submitted input, and queue entries never enter SQLite, diagnostics, restore, or a replacement process.
...
Read-only workspace enumeration returns schema-version-4 `WorkspaceMetadata` ordered by `UpdatedAtUtc` descending, `CreatedAtUtc` descending, then stable ID. It changes no timestamps, status, lifecycle, alert, or metadata facts. In-app preloading never seeds a missing target; only a completed activation updates recency. Missing directories remain metadata-only and unavailable.

At startup, stale `Reserved` deliveries with no successful-write marker return to `Pending`; a reservation durably marked with a successful terminal write is finalized as `Delivered` instead. Pending alerts remain eligible but are not sent until the matching parent Planner is newly running, its readiness adapter reports safe input, and no partial input exists. Reservation is an atomic compare-and-update. A known pre-write rejection releases the record, an indeterminate terminal-write outcome fails closed, and a successful write records its existing `DeliveredAtUtc` fact before the final `Delivered` state transition. Final commit failure never releases a known-successful write. Delivered records never become eligible again, and schema version remains 4 because the existing delivery columns represent these facts without a migration.

Live completions for the same parent use a fixed 250 ms event-driven coalescing window anchored to the first eligible completion. Later completions strictly before that deadline join the batch; completions at or after it start the next window and never extend the prior deadline. Prepared batches retain exact membership while waiting for their parent Planner. All members of a batch are reserved, committed, or released together in one SQLite transaction, while remaining independent lifecycle and delivery records. The message is deterministic and contains only persisted sub-agent display names. No polling, terminal content, or model-generated summary participates in batching. Restored pending events may also form one batch once their newly started parent becomes safe for input.
