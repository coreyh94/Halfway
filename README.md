...
Metadata is stored in `%LOCALAPPDATA%\Halfway\halfway.db`. Schema version 4 retains all schema version 3 workspace, session, relationship, lifecycle-event, and alert-delivery data and transactionally adds application-run records. Each run has a stable ID, start time, optional clean-shutdown time, and application version. At startup, only an unfinished immediately preceding run is reported as evidence that Halfway itself did not shut down cleanly; a later clean run prevents older unfinished history from being repeatedly reported. The current run is marked clean only after orderly session shutdown and pending lifecycle persistence complete.
