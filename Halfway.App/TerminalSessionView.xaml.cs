...
public sealed partial class TerminalSessionView : UserControl
{
    ...
    public void SetStatus(AgentStatus status) { StatusText.Text=status.ToString().ToUpperInvariant(); var active=status is AgentStatus.Queued or AgentStatus.Running or AgentStatus.Waiting; StartButton.IsEnabled=!active; StartButton.Content=status == AgentStatus.Disconnected ? "Restart" : "Start"; StopButton.IsEnabled=active; InputText.IsEnabled=status==AgentStatus.Running; }
